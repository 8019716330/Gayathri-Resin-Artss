...
$.each(sectionArray, function(index, value){
    ...
    var navLinkId = '.nav-link-' + value;

    $(document).scroll(function(){
        var offsetSection = $(sectionId).offset().top - 154;
        var docScroll = $(document).scrollTop();
        var docScroll1 = docScroll + 1;
    ...
    $(navLinkId).click(function(e){
        var offsetClick = $(sectionId).offset().top - 154;
        e.preventDefault();
        $('html, body').animate({
            'scrollTop': offsetClick
        }, 300)
...
$(document).ready(function(){
    ...
    $('.nav-link-1').removeClass('inactive');
});